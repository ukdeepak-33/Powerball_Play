...
def debug_data_health_route():
    try:
        return jsonify({
            'df_row_count': len(df) if df is not None else None,
            'df_empty': df.empty if df is not None else None,
            'historical_white_ball_sets_count': len(historical_white_ball_sets) if historical_white_ball_sets else 0,
            'last_analysis_cache_update': str(last_analysis_cache_update) if 'last_analysis_cache_update' in globals() else None,
            'analysis_cache_keys': list(analysis_cache.keys()) if 'analysis_cache' in globals() else None,
