...
def generate_powerball_numbers(df_source, group_a, odd_even_choice, combo_choice, white_ball_range, powerball_range, excluded_numbers, high_low_balance=None, selected_sum_range_tuple=None, is_simulation=False):
    ...
    max_attempts = 5000 # Increased attempts for stricter adherence
    attempts = 0
    ...
    while attempts < max_attempts:
        ...
        # 4. Check Group A constraint
        group_a_numbers = [num for num in white_balls_candidate if num in group_a]
        if len(group_a_numbers) < 2: # Assuming at least 2 Group A numbers is a general rule, adjust if needed
            attempts += 1
            continue
...
def get_consecutive_trends_for_df(df_to_analyze):
    ...
    for idx, row in df_to_analyze.iterrows():
        ...
        # FIX: Changed from 'consecutive_pairs' to 'consecutive_sequences'
        trend_data.append({
            'draw_date': row['Draw Date_dt'].strftime('%Y-%m-%d'),
            'consecutive_present': "Yes" if consecutive_sequences else "No", 
...
def get_consecutive_numbers_yearly_trends(df_source):
    ...
    return {
        ...
        'all_consecutive_pairs_flat': flat_consecutive_sequences_list # Changed key to all_consecutive_pairs_flat for frontend compatibility
    }
...
def _summarize_for_ai(df_source):
    ...
    monthly_trends_data = get_monthly_white_ball_analysis_data(df_source, num_months_for_top_display=6) # Limiting to 6 months for summary
    
    recent_monthly_numbers_wb = defaultdict(int)
    for month_data in monthly_trends_data['monthly_data']:
        for wb_data in month_data['drawn_white_balls_with_counts']:
            recent_monthly_numbers_wb[wb_data['number']] += wb_data['count']
    ...
    if sorted_recent_monthly_wb:
        summary_parts.append("White Balls frequently drawn in recent 6-month period: " + ", ".join([f"{n} ({c} times)" for n, c in sorted_recent_monthly_wb[:15]])) 
    ...
    if consecutive_trends_list:
        ...
        summary_parts.append(f"Consecutive numbers appeared in {consecutive_percentage:.2f}% of recent draws (last 12 months).")
...
def consecutive_trends_route():
    ...
    all_consecutive_sequences_flat = yearly_consecutive_data_full['all_consecutive_pairs_flat'] # Corrected variable name
    ...
    return render_template('consecutive_trends.html',
                           ...
                           all_consecutive_pairs_flat=all_consecutive_sequences_flat) # Pass the corrected variable

# NEW API endpoint for yearly consecutive trends (now returns flat list)
@app.route('/api/consecutive_trends_by_year/<int:year>') # Corrected endpoint name as per frontend's fetch call
def api_consecutive_trends_by_year(year):
    if df.empty:
        return jsonify({"error": "Historical data not loaded or is empty."}), 500

    # Filter the main DataFrame for the requested year
    df_for_year = df[df['Draw Date_dt'].dt.year == year].copy()
    
    # Calculate consecutive trends for the filtered year's data
    trends_for_year = get_consecutive_trends_for_df(df_for_year.sort_values(by='Draw Date_dt', ascending=False))
    
    return jsonify(trends_for_year)


# NEW ROUTE: For the AI-Powered Smart Pick Generator on the homepage
@app.route('/generate_smart_picks', methods=['POST'])
def generate_ai_smart_picks(): 
    if df.empty:
        return jsonify({"error": "Historical data not loaded. Please try again later."}), 500

    try:
        # Parse incoming JSON data from the homepage's fetch request
        request_data = request.get_json()
        num_sets_to_generate = int(request_data.get('num_sets', 1))

        # Default values for parameters not provided by the homepage's AI generator UI.
        # These are sensible defaults for an "AI-Powered" generator.
        excluded_numbers_ai = []
        num_from_group_a_ai = 0 
        odd_even_choice_ai = 'Any'
        sum_range_tuple_ai = None 
        prioritize_monthly_hot_ai = True 
        prioritize_grouped_patterns_ai = True 
        prioritize_special_patterns_ai = True 
        prioritize_consecutive_patterns_ai = True 
        force_specific_pattern_ai = []

        # Call the core `generate_smart_picks` function (from the original smart_pick_generator.py logic)
        # You would replace `generate_smart_picks_core_logic` with the actual function name from your other file
        # or consolidate that logic here. For now, using a placeholder.
        
        # NOTE: If your actual `generate_smart_picks` function expects `df_source` as first arg, ensure `df` is passed.
        generated_sets = generate_smart_picks_core_logic( # Placeholder - REPLACE THIS with your actual smart pick generation function
            df_source=df, # Pass the global df
            num_sets=num_sets_to_generate,
            excluded_numbers=excluded_numbers_ai,
            num_from_group_a=num_from_group_a_ai,
            odd_even_choice=odd_even_choice_ai,
            sum_range_tuple=sum_range_tuple_ai,
            prioritize_monthly_hot=prioritize_monthly_hot_ai,
            prioritize_grouped_patterns=prioritize_grouped_patterns_ai,
            prioritize_special_patterns=prioritize_special_patterns_ai,
            prioritize_consecutive_patterns=prioritize_consecutive_patterns_ai,
            force_specific_pattern=force_specific_pattern_ai
        )
        
        # For display on the homepage, get last draw dates for the *last* generated set
        last_draw_dates = {}
        if generated_sets:
...
# Placeholder for your actual smart pick generation function if it's external
# You must ensure this function is defined or imported correctly.
def generate_smart_picks_core_logic(df_source, num_sets, excluded_numbers, num_from_group_a, odd_even_choice, sum_range_tuple, prioritize_monthly_hot, prioritize_grouped_patterns, prioritize_special_patterns, prioritize_consecutive_patterns, force_specific_pattern):
    # This is a dummy implementation. REPLACE with your actual complex logic
    # from your smart_pick_generator.py (if it's a separate function, bring it here or import it)
    generated_sets = []
...
def smart_pick_generator_route():
    # This is the route for your 'Pattern-Based Smart Pick Generator' HTML page
    return render_template('smart_pick_generator.html', sum_ranges=SUM_RANGES, group_a=group_a)

# The form POST for the 'Pattern-Based Smart Pick Generator'
@app.route('/generate_smart_picks_route', methods=['POST'])
def generate_smart_picks_submission_route(): # Renamed to differentiate from the homepage's AJAX route
    if df.empty:
    ...
    try:
        ...
        # Call the core smart pick generation function
        generated_sets = generate_smart_picks_core_logic( # Using the placeholder/actual core logic
            df_source=df,
            num_sets=num_sets_to_generate,
